...
public class DownloadManager {
  ...
  private Box<UnLimit91PornItem> box = MyApplication.getInstace().getBoxStore()
  ...
  public int startDownload1(final String url, final String path) {
    return FileDownloader.getImpl().create(url).setPath(path).setListener(lis)
        .setWifiRequired(false).start();
  }

  public int startDownload(String url, final String path) {
    int id = FileDownloader.getImpl().create(url).setPath(path).setListener(lis)
        .setWifiRequired(false).setAutoRetryTimes(1).asInQueueTask().enqueue();
  ...
  private FileDownloadListener lis = new FileDownloadListener() {
    ...
    protected void pending(BaseDownloadTask task, int soFarBytes, int totalBytes) {
      Logger.t(TAG).d(
          "pending:" + "--status:" + task.getStatus() + "--:soFarBytes：" + soFarBytes
    ...
    protected void started(BaseDownloadTask task) {
      super.started(task);
      Logger.t(TAG).d(
          "started:" + "--status:" + task.getStatus() + "--:soFarBytes："
              + task.getSmallFileSoFarBytes() + "--:totalBytes：" + task.getSmallFileTotalBytes());
      saveDownloadInfo(task);
      update(task);
    }
    ...
    protected void connected(BaseDownloadTask task, String etag, boolean isContinue,
      ...
      Logger.t(TAG).d(
          "connected:" + "--status:" + task.getStatus() + "--:soFarBytes：" + soFarBytes
              + "--:totalBytes：" + totalBytes);
      saveDownloadInfo(task);
      update(task);
    }
    ...
    protected void blockComplete(BaseDownloadTask task) {
      Logger.t(TAG).d(
          "blockComplete:" + "--status:" + task.getStatus() + "--:soFarBytes："
              + task.getSmallFileSoFarBytes() + "--:totalBytes：" + task.getSmallFileTotalBytes());
      // blockComplete(task);
    }
    ...
    protected void completed(BaseDownloadTask task) {
      Logger.t(TAG).d(
          "completed:" + "--status:" + task.getStatus() + "--:soFarBytes："
              + task.getSmallFileSoFarBytes() + "--:totalBytes：" + task.getSmallFileTotalBytes());
      Logger.d("completed");
      saveDownloadInfo(task);
      update(task);
    ...
    protected void paused(BaseDownloadTask task, int soFarBytes, int totalBytes) {
      Logger.t(TAG).d(
          "paused:" + "--status:" + task.getStatus() + "--:soFarBytes：" + soFarBytes
              + "--:totalBytes：" + totalBytes);
      saveDownloadInfo(task);
      update(task);
    }
    ...
    protected void error(BaseDownloadTask task, Throwable e) {
      Logger.t(TAG).d(
          "error:" + "--status:" + task.getStatus() + "--:soFarBytes："
              + task.getSmallFileSoFarBytes() + "--:totalBytes：" + task.getSmallFileTotalBytes());
      saveDownloadInfo(task);
      update(task);
    }
    ...
    protected void warn(BaseDownloadTask task) {
      Logger.t(TAG).d(
          "warn:" + "--status:" + task.getStatus() + "--:soFarBytes："
              + task.getSmallFileSoFarBytes() + "--:totalBytes：" + task.getSmallFileTotalBytes());
      update(task);
    }
  };
  ...
  private void blockComplete(final BaseDownloadTask task) {
    final List<DownloadStatusUpdater> updaterListCopy = (List<DownloadStatusUpdater>) updaterList
        .clone();

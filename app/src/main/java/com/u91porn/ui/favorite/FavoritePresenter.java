...
public class FavoritePresenter extends MvpBasePresenter<FavoriteView> implements IFavorite {
  ...
  public void loadFavoriteData(int skip, int pageSize) {
    List<UnLimit91PornItem> unLimit91PornItemList = unLimit91PornItemBox.query()
        .equal(UnLimit91PornItem_.favorite, UnLimit91PornItem.FAVORITE_YES)
        .orderDesc(UnLimit91PornItem_.favoriteDate).build().find(skip, pageSize);
    if (isViewAttached()) {
        ...
        .create(new ObservableOnSubscribe<List<UnLimit91PornItem>>() {
          ...
          public void subscribe(ObservableEmitter<List<UnLimit91PornItem>> e) throws Exception {
            List<UnLimit91PornItem> unLimit91PornItems = unLimit91PornItemBox.query()
                .equal(UnLimit91PornItem_.favorite, UnLimit91PornItem.FAVORITE_YES)
                .orderDesc(UnLimit91PornItem_.favoriteDate).build().find();

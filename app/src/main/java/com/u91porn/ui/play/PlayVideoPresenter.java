...
public class PlayVideoPresenter extends MvpBasePresenter<PlayVideoView> implements IPlay {
  ...
  public void loadVideoUrl(String viewKey) {
    ...
    MyApplication
        .getInstace()
        .getCacheProviders()
        .getVideoPlayPage(mNoLimit91PornServiceApi.getVideoPlayPage(viewKey, ip),
            new DynamicKey(viewKey), new EvictDynamicKey(false)).compose(getView().bindView())
        ...
        }).map(ParseUtils::parseVideoPlayUrl).subscribeOn(Schedulers.io())

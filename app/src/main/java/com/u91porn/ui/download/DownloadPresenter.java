...
public class DownloadPresenter extends MvpBasePresenter<DownloadView> implements IDownload {
  ...
  private Box<UnLimit91PornItem> unLimit91PornItemBox = MyApplication.getInstace().getBoxStore()
  ...
  public void loadDownloadingData() {
    List<UnLimit91PornItem> unLimit91PornItems = unLimit91PornItemBox.query()
        .notEqual(UnLimit91PornItem_.status, FileDownloadStatus.completed).and()
        .notEqual(UnLimit91PornItem_.downloadId, 0).orderDesc(UnLimit91PornItem_.addDownloadDate)
        .build().find();
    if (isViewAttached()) {
  ...
  public void loadFinishedData() {
    List<UnLimit91PornItem> unLimit91PornItems = unLimit91PornItemBox.query()
        .equal(UnLimit91PornItem_.status, FileDownloadStatus.completed)
        .orderDesc(UnLimit91PornItem_.finshedDownloadDate).build().find();
    if (isViewAttached()) {
   ...
   * @param videoUrl
   */
  private void copyCacheFile(final String videoUrl, final DownloadListener downloadListener)
      throws IOException {
    Observable
        .create(new ObservableOnSubscribe<File>() {
          ...
          public void subscribe(ObservableEmitter<File> e) throws Exception {
            ...
            File fromFile = new File(MyApplication.getInstace().getExternalCacheDir()
                + "/video-cache/" + cacheFileName);
        ...
        .map(new Function<UnLimit91PornItem, String>() {
          ...
          public String apply(UnLimit91PornItem unLimit91PornItem) throws Exception {
            ...
            unLimit91PornItem.setDownloadId(FileDownloadUtils.generateId(
                unLimit91PornItem.getVideoUrl(), unLimit91PornItem.getDownLoadPath()));
            MyApplication.getInstace().getBoxStore().boxFor(UnLimit91PornItem.class)
                .put(unLimit91PornItem);
            return "下载完成";
          }
    ...
    // UnLimit91PornItem unLimit91PornItem =
    // BoxQureyHelper.findByVideoUrl(videoUrl);
    // MyFileNameGenerator myFileNameGenerator = new MyFileNameGenerator();
    // String cacheFileName = myFileNameGenerator.generate(videoUrl);
    // File fromFile = new File(MyApplication.getInstace().getExternalCacheDir()
    // + "/video-cache/" + cacheFileName);
    ...
    // unLimit91PornItem.setProgress(100);
    // unLimit91PornItem.setFinshedDownloadDate(new Date());
    // unLimit91PornItem.setDownloadId(FileDownloadUtils.generateId(unLimit91PornItem.getVideoUrl(),
    // unLimit91PornItem.getDownLoadPath()));
